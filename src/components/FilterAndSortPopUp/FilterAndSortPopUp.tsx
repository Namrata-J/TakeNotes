...
const FilterAndSortPopUp = () => {
    ...
    const labelsReducer = (acc: any[], curr: any) => !acc.includes(curr.label) ? [...acc, curr.label] : acc

    const uniqueLabels = labeledNotes.reduce(labelsReducer, []);

    return (
        <div
            className="tn_filter-popUp-outerContainer popUp-outerContainer"
            style={{ display: popUpFilterAndSortDisplay }}>
            <div className="tn_filter-popUp-container popUp b-rad1">
                <div className="tn_filter-subcontainer1">
                    <h4>Sort & Filter Notes</h4>
                    <GrFormClose
                        className="tn_filter-popUp-closeBtn"
                        onClick={
                            () => {
                                filterAndSortDispatch({ type: "CLEAR_FILTERS" });
                ...
                <div className="tn_filter-subcontainer2">
                    ...
                    <select
                        name="sortNotes"
                        id="tn_sort"
                        value={filterAndSortState.SortByDate}
                        onChange={
                            (e) => filterAndSortDispatch(
                                {
                                    type: "SORT_BY_DATE",
                    ...
                    <select
                        name="filterNotes"
                        id="tn_filter"
                        value={filterAndSortState.FilterByCategory}
                        onChange={
                            (e) => filterAndSortDispatch(
                                {
                                        ...
                                        return (
                                            <label className="tn_select-label" htmlFor={label} key={index}>
                                                <input
                                                    ...
                                                    id={label}
                                                    value={label}
                                                    checked={filterAndSortState.FilterByLabels.includes(label)}
                                                    onChange={
                                                        (e) => filterAndSortDispatch(
                                                            {
                ...
                <div className="tn_filter-subcontainer3">
                    <button
                        className="tn_popUp-reset-btn et_p-simple-btn primary-color btn b-rad1"
                        onClick={(e) => filterAndSortDispatch({ type: "CLEAR_FILTERS" })} >
                        Reset
                    </button>
                    <button
                        className="tn_popUp-done-btn et_p-simple-btn primary-color btn b-rad1"
                        onClick={() => closeThePopUpFilterAndSort()} >
                        Done
                    </button>
                </div>
            </div>
